  ...
  <div class="artists_list_page">
    <div class="instruction_content">
      ...
      <div class="instructions">
        <ol>
          <li>
            Fetch all the artists with an HTTP Request. This
            request has to be done when the page is created. Save the
            list of artists in the 'artists' variable.
          </li>
          <li>
            Complete the function add movie. When the request has been received,
    ...
    <md-button
      class="md-icon-button md-raised md-primary add_artist_btn"
...
export default {
  ...
  mounted() {
    // this.axios.get("http://localhost:3736/api/artists/").then(response => {
    ...
    this.axios.get("http://localhost:3736/api/artists/").then(response => {
      for (var i in response.data.data) {
        this.artists.push(response.data.data[i]);
      }
    });
  },
  methods: {
    ...
    deleteArtistById(artistId) {
      // TODO question 3
      this.axios
        .delete("http://localhost:3736/api/artists/" + artistId);
    }
  }
};
</script>
